...
function walletConnect(options) {
  ...
  return () => {
    return {
      label: "WalletConnect",
      getIcon: async () => (await import("./icon-ZTZVGIXP.js")).default,
      getInterface: async ({ chains, EventEmitter, appMetadata }) => {
        const { ProviderRpcError, ProviderRpcErrorCode } = await import("./dist-QR6QBLDX.js");
        const { default: EthereumProvider, REQUIRED_METHODS } = await import("./index.es-YTUK2K43.js");
        const { Subject, fromEvent } = await import("./esm5-HCZAFDAN.js");
        const { takeUntil, take } = await import("./operators-QB7DIC4O.js");
        const getMetaData = () => {
          if (!appMetadata)
            return void 0;
