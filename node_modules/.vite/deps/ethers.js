...
var Wallet = class _Wallet extends Signer {
  ...
  static fromMnemonic(mnemonic, path, wordlist) {
    ...
    return new _Wallet(HDNode.fromMnemonic(mnemonic, null, wordlist).derivePath(path));
  }
};
...
var Resolver = class {
  ...
  _getAddress(coinType, hexBytes) {
    ...
    if (coinInfo.prefix != null) {
      ...
      if (version7 >= 0 && bytes.length === 2 + length && length >= 1 && length <= 75) {
...
try {
  ...
  const logger21 = new Logger(version3);
  WS = function() {
    logger21.throwError("WebSockets not supported in this environment", Logger.errors.UNSUPPORTED_OPERATION, {
  ...
  for (let i = 0; i < networks.length; i++) {
    ...
    if (result) {
      if (!(result.name === network.name && result.chainId === network.chainId && (result.ensAddress === network.ensAddress || result.ensAddress == null && network.ensAddress == null))) {
        logger12.throwArgumentError("provider mismatch", "networks", networks);
      }
    } else {
      result = network;
    }
...
function _pack(type, value, isArray) {
  ...
  if (match) {
    ...
    if (match[2] && String(size) !== match[2] || size % 8 !== 0 || size === 0 || size > 256) {
      logger18.throwArgumentError("invalid number type", "type", type);
    }
    if (isArray) {
      size = 256;
    }
  ...
  if (match) {
    ...
    if (String(size) !== match[1] || size === 0 || size > 32) {
      logger18.throwArgumentError("invalid bytes type", "type", type);
    }
    if (arrayify(value).byteLength !== size) {
      logger18.throwArgumentError(`invalid value for ${type}`, "value", value);
    }
    if (isArray) {
  ...
  if (match && Array.isArray(value)) {
    ...
    if (count != value.length) {
      logger18.throwArgumentError(`invalid array length for ${type}`, "value", value);
    }
    const result = [];
  ...
  return logger18.throwArgumentError("invalid type", "type", type);
}
function pack(types, values) {
  if (types.length != values.length) {
    logger18.throwArgumentError("wrong number of values; expected ${ types.length }", "values", values);
  }
  const tight = [];
...
var version6 = "ethers/5.7.2";

// node_modules/ethers/lib.esm/ethers.js
var logger20 = new Logger(version6);
